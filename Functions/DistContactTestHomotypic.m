function [dfcell] = DistContactTestHomotypic(ObjStruc,SearchI,thresholdarr)
%In this version, the image subtraction is performed here instead of in the
%main body
...
for p = 1:size(ObjStruc,1)
    ...
    %image subtraction, skeletonize, then generate list again
    for r = 1:size(tplist,1)
        tempim(tplist(r,1),tplist(r,2)) = 1;
    end
    tempsim = imbinarize(tempsim - tempim);
    ...
    for r = 1:size(tplist,1)
        ...
        sbox = tempsim(tplist(r,1)-thresholdarr(2):tplist(r,1)+thresholdarr(2),tplist(r,2)-thresholdarr(2):tplist(r,2)+thresholdarr(2));
        [sr,sc] = find(sbox);
        if ~isempty(sr)
            for s = 1:size(sr,1)
